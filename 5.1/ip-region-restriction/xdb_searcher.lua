...
function _M:search(ip_src)
    ...
    local il0 = BIT32.band(BIT32.rshift(ip, 24), 0xFF)
    --local il1 = (ip >> 16) & 0xFF
    local il1 = BIT32.band(BIT32.rshift(ip, 16), 0xFF)
...
function _M.check_ip(ip_str)
    ...
    for p in string.gmatch(ip_str..".", "([%d]+)%.") do
        ...
        ip = BIT32.bor(ip, BIT32.lshift(v, offset_arr[id]))
        id = id + 1
    end
...
function getLong(buff, idx)
    ...
    local i2 = BIT32.lshift(string.byte(string.sub(buff, idx+1, idx+1)), 8)
    --local i3 = (string.byte(string.sub(buff, idx+2, idx+2)) << 16)
    local i3 = BIT32.lshift(string.byte(string.sub(buff, idx+2, idx+2)), 16)
    --local i4 = (string.byte(string.sub(buff, idx+3, idx+3)) << 24)
    local i4 = BIT32.lshift(string.byte(string.sub(buff, idx+3, idx+3)), 24)
    --return (i1 | i2 | i3 | i4)
    return BIT32.bor( BIT32.bor(BIT32.bor(i1, i2), i3), i4)
end
